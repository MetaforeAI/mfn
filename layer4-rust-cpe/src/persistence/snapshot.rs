/// LMDB-based snapshot creator for Layer 4
///
/// Creates periodic full snapshots of patterns for fast recovery.
...
/// Serializable snapshot of a similarity well
...
pub struct WellSnapshot {
    ...
    pub last_accessed_timestamp_ms: u64,
}
...
impl SnapshotCreator {
    ...
    pub fn new(path: impl AsRef<Path>) -> Result<Self> {
        ...
        let db = {
            ...
            let db = unsafe {
                txn.create_db(Some("wells"), lmdb::DatabaseFlags::empty())
...
mod tests {
    ...
    fn test_snapshot_create_and_load() {
        ...
        wells.insert(MID_3, create_test_well(MID_3, "memory 3"));
        ...
        assert_eq!(loaded.get(&MID_1).unwrap().content, "memory 1");
        assert_eq!(loaded.get(&MID_2).unwrap().content, "memory 2");
        assert_eq!(loaded.get(&MID_3).unwrap().content, "memory 3");
    }
    ...
    fn test_snapshot_metadata() {
        ...
        // Create snapshot
        let mut wells = HashMap::new();
        wells.insert(MID_1, create_test_well(MID_1, "test"));
        creator.create_snapshot(&wells).unwrap();
